    ...
    var view = {
      ...
      init: function (options) {
        ...
        if (!view.item.hasClass('no-prettify')) {
          ...
          if (tagname == "select" && view.item.attr('multiple') === undefined) {
            view.type = 'select';
            view._renderSelect();
          }
          else if (tagname == "select" && view.item.attr('multiple') !== undefined) {
            view.type = 'select';
            view._renderMultipleSelect();
          }
      ...
      _renderMultipleSelect: function () {
        view.item.wrap("<label class='select prettify prettify-multiple'></label>");
        view.label = view.item.parent();
      ...
      _renderFile: function () {
        ...
        if (view.item.is(':disabled')) {
          view.label.addClass('disabled');
        }

        if (view.item.hasClass('error')) {
          view.label.addClass('error');
        }

        var fake = $('<div><span><a class="button">' + Drupal.t('Choose a file') + '</a></span><span class="filetext"></span></div>');
